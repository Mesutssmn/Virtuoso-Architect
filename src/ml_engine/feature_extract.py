...
def extract_features_batch(midi_files, output_csv=None, n_jobs=None, save_interval=100):
    """
    ...
    try:
        ...
        # Close pool normally if not interrupted
        if not interrupted:
            pool.close()
            pool.join()
        else:
    ...
    print(f"  💻 Using {n_jobs} CPU cores (out of {mp.cpu_count()} available)")
    print(f"  📊 Processing {len(midi_files)} MIDI files...")
    print(f"  ℹ️  Press Ctrl+C to stop and save partial results")
    ...
    # Save results
    if interrupted:
        print(f"\n📊 Processed {len(results)} files before interruption")
        if results and output_csv:
            print(f"💾 Saving {len(results)} processed files...")
            df_partial = pd.DataFrame(results)
